 ...
 */
public class Pacote {
  public SegmentoTcp2 segmento = null;
  
  public Pacote(SegmentoTcp2 segmento) {
   ...
   * @param dados = dados
   */
  public Pacote(int portaOrigem, int portaDestino, int numeroDeSequencia, int numeroAck, int ack,
      int syn, int fin, int tamanhoJanelaReceptor, int tamanhoDados, byte[] dados) {
    // TODO Auto-generated constructor stub
    this.segmento = new SegmentoTcp2((char) portaOrigem, (char) portaDestino, numeroDeSequencia,
        numeroAck, (byte) ack, (byte) syn, (byte) fin, (char) tamanhoJanelaReceptor, tamanhoDados);
    this.segmento.dados = dados;
  }
  ...
  public Pacote(byte[] segmento) {
    ...
    this.segmento = new SegmentoTcp2(bufferPortaOrigem.getChar(), bufferPortaDestino.getChar(),
        bufferNumeroDeSequencia.getInt(), bufferNumeroAck.getInt(), bufferAck.get(),
        bufferSyn.get(), bufferFin.get(), bufferTamanhoJanelaReceptor.getChar(),
        bufferTamanhoDados.getInt());
    
    if (this.size() > 0) {
      this.segmento.dados = new byte[segmento.length - offset];
      System.arraycopy(segmento, offset, this.segmento.dados, 0, this.segmento.dados.length);
    }
  }
  ...
  public byte[] toBytes() {
    ...
    ByteBuffer bufferRetorno = ByteBuffer.allocate(Pacote.headerSize() + this.size());
    ...
    bufferRetorno.put(segmento.getAck());
    bufferRetorno.put(segmento.getSyn());
    bufferRetorno.put(segmento.getFin());
    bufferRetorno.putChar(segmento.getTamanhoJanelaReceptor());
    bufferRetorno.putInt(segmento.getTamanhoDados());
    if (this.size() > 0) {
      bufferRetorno.put(segmento.dados);
    }
  ...
  public int size() {
    return segmento.getTamanhoDados();
  }
}
